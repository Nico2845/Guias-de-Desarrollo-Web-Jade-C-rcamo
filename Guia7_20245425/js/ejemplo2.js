...
const validarFormulario = function () {
    ...
    // Controles por nombre para grupos de Radio/Checkbox
    ...
    // 1. Validación de campos de texto vacíos (Punto a)
    ...
    // 2. Validación de fecha de nacimiento (Punto b)
    validarFechaNacimiento(fechaNac);

    // 3. Validación de correo electrónico (Punto c)
    validarCorreoElectronico(correo);

    // 4. Validación de contraseñas iguales (Punto d)
    validarContrasenasIguales(password, repetirPassword);

    // 5. Validación de "algunos intereses" (Checkbox) (Punto e)

    validarGrupoCheckbox(["idCkProgramacion", "idCkBD", "idCkRedes", "idCkSeguridad"], "Intereses");

    // 6. Validación de carrera (Radio) (Punto f)
    validarGrupoRadio("idRdCarrera", "Carrera");

    // 7. Validación de país de origen (Select) (Punto g)
    validarSelect(pais, "País de origen");

    // Si hay errores, mostrarlos. Si no, generar la tabla de resultados.
    if (errores.length > 0) {
        mostrarErrores();
        return false;
    } else {
